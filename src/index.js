...
const createSMTPWindow = (parentWindow) => {
  ...
  ipcMain.on('update-smtp-json', (event, args) => {
    ...
    .update({ host: obj.host, username: obj.username, password: obj.password, port: obj.port}).then(function(evt, rows){
      ...
      SMTPWindow.webContents.send("updateResponse")
      updateHiddenInput(parentWindow);
      event.reply("updateResponse-reply", "Successfully Updated.")
      
    })

  });

  ipcMain.on('verify-smtp-in-smtpwin', (event, smtpCred) => {
      let resultSMTP = getSMTPTable();

      resultSMTP.then(function(rows){
        try {
          event.reply("smtpwin-row-request", rows)
          // console.log(rows)
        } catch (e) {
          console.log(e);
        }
...
ipcMain.on('excel-file', (event, args) => {
  ...
  data.forEach(dt => {
    if(`${dt["Email Address"]}` !== "undefined"){
      getemail.push(`${dt["Email Address"]}`)
    }
  })


  event.reply('res-upload-excel',getemail.length)
})

ipcMain.on('send-mail-trigger-SMTPTable_result', (event) => {
    ...
    let resultSMTP = getSMTPTable();
      ...
      resultSMTP.then(function(rows){
        try {
          ...
          objSMTP.host = rows[0].host;
          objSMTP.username = rows[0].username;
          objSMTP.password = rows[0].password;
          objSMTP.port = rows[0].port;
          ...
          nodemailer.createTransport({
            ...
            auth: {
              user: rows[0].username, // generated ethereal user
              pass: rows[0].password, // generated ethereal password
            },
          }).verify(function(error, success) {
            if (error) {
              responseMessage = error.message;
              event.reply("SMTPRow-Response-error", error.message)
            } else {
              console.log("Server is ready to take our messages");
              // console.log(success);
              // responseMessage = "Server is ready to take our messages";
              event.reply("SMTPRow-Response", rows, getExcelData())
            }
          })
